...
/**
 * 統合設定ページ（user / admin 共用）。
 * 文字起こし言語と MCP credentials を管理する。
 */
export default function SettingsPage() {
  ...
  const handleIssueMcp = async () => {
    ...
    setMcpBusy(true);
    try {
      const res = await fetch('/api/web/mcp-credentials', { method: 'POST' });
  ...
  const handleRevokeMcp = async () => {
    ...
    setMcpBusy(true);
    try {
      await fetch('/api/web/mcp-credentials', { method: 'DELETE' });
      setMcpIssued(null);
      loadMcp();
    } finally {
      setMcpBusy(false);
